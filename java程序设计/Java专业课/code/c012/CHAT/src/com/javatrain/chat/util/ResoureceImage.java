...
//图标面板
public class ResoureceImage extends JDialog {
	...
	public ResoureceImage() {
		initUI();
	}

	public ResoureceImage(LoginFrame loginFrame) {
		super(loginFrame);
		...
		SwingUtilities.convertPointToScreen(point, loginFrame);
		point.x = point.x + (int) (loginFrame.getWidth() - this.getWidth()) / 2;
		point.y = point.y + (int) (loginFrame.getHeight() - this.getHeight())
				/ 2;
	...
	class ImageJPanel extends JPanel {
		...
		ImageJPanel() {
			ImageBox jlImage = null;
			List<ImageIcon> imageList = ImageFactory.getInstance()
					.getimageIconList();
			int size = imageList.size();
			size = ImageFactory.getInstance().getHm_imageIcon().size();
			ImageIcon imageIcon = null;
			int row = size % col == 0 ? size / col : size / col + 1;
			// System.out.println(size + "  " + row);
			GridLayout gridLayout = new GridLayout(row, col, 30, 20);
			// FlowLayout flowLayou = new FlowLayout(FlowLayout.LEFT, 10, 10);
			setLayout(gridLayout);

			for (int i = 1; i <= size; i++) {
				imageIcon = ImageFactory.getInstance().getImageIndex(i + "");
				jlImage = new ImageBox((i) + "", imageIcon);
				jlImage.setPreferredSize(new Dimension(25, 60));
