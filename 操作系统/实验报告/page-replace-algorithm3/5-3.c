...
void run(){
	...
	fprintf(e,"NAME   size  perm     INDEX_ADDRESS\n");
	for(x1=0;x1<N+1;x1++)//4，一下是第4步骤
		fprintf(e,"      %c    %d    %d      %x\n",HEAD->names[x1],HEAD->size[x1],HEAD->perm[x1],HEAD->p[x1]);//5
...
void main(){
	...
	while(kk<jobs){
		...
		printf("perm:");  // 5
		scanf("%d",&(job[kk].perm));  // 5
		scanf("%c",&bb);  // 1
		
		fprintf(e,"%c,%d,%d",job[kk].name,job[kk].size,job[kk].perm);//1
		kk++;
	}
