...
(function (global, factory) {
	...
	function normalizeLocation(raw, current, append, router) {
		...
		if (!next.path && next.params && current) {
			...
			if (current.name) {
				...
				var rawPath =
					current.matched[current.matched.length - 1].path;
...

		{
			...
			var found = pathList
				...
				.filter(function (path) {
					return (
						path && path.charAt(0) !== "*" && path.charAt(0) !== "/"
					);
				});
	...
	History.prototype.confirmTransition = function confirmTransition(
		...
		runQueue(queue, iterator, function () {
			...
			runQueue(queue, iterator, function () {
				if (this$1.pending !== route) {
					return abort(
						createNavigationCancelledError(current, route)
					);
				}
				this$1.pending = null;
				onComplete(route);
	...
	function getHash() {
		...
		if (searchIndex < 0) {
			...
			href =
				decodeURI(href.slice(0, searchIndex)) +
				href.slice(searchIndex);
		}
