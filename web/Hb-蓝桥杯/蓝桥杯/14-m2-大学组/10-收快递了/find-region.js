...
function findRegion1(regions, regionName) {
	// TODO: 在这里写入具体的实现逻辑
...
function findRegion(regions, regionName) {
	...
	regions.forEach((region) => {
		console.log('in---', [region.name])
		let res = findRegion(region.children, regionName);
		console.log('out---', [region.name])
		if (res)
			now = [region.name, ...res]
...
console.time('aa'); // 记录时间开始
console.log(findRegion(regions1, "雷波县"))
console.timeEnd('aa'); // 记录时间结束并输出时间差
