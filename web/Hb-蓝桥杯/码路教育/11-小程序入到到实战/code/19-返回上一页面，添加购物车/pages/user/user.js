...
Page({
  ...
  async goToLogin(){
    ...
    if(res.errno ===  0){
      ...
      // if(getApp().globalData.isGoBack){
      //   // 5)跳到上一页
      //   wx.navigateTo({
      //     url: getApp().globalData.prevRoute+"?id="+getApp().globalData.detailsId
      //   })
      //   getApp().globalData.isGoBack = false;
      // }

      if(getApp().globalData.isGoBack){
        if(getApp().globalData.isPageFromTabbar){
          // 上一页面是tabbar页
          wx.switchTab({
            url: getApp().globalData.prevRoute,
          })
          getApp().globalData.isPageFromTabbar = false;
        }else{
          wx.navigateTo({
            url: getApp().globalData.prevRoute+"?id="+getApp().globalData.detailsId
          })
