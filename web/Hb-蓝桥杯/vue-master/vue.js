...
(function(global, factory) {
	...
	function makeMap(str,
		...
		for (var i = 0; i < list.length; i++) {
			map[list[i]] =
			true; //map 对象中的[name1,name2,name3,name4]  变成这样的map{name1:true,name2:true,name3:true,name4:true}
		}
	...
	function validateProp(
		...
		if (booleanIndex > -1) { //如果是boolean值
			...
			if (absent && !hasOwn(prop,
				'default')) { //如果key 不是propsData 实例化，或者 没有定义default 默认值的时候   设置value 为false
				value = false;
			} else if (
				value === '' //如果value 是空
				||
	...
	function assertType(value, type) {
		...
		if (simpleCheckRE.test(
			expectedType)) { //type 必须是String|Number|Boolean|Function|Symbol 构造函数 这里才为真 (String|Number|Boolean|Function|Symbol)
			var t = typeof value;
			//转换成小写
		...
		if (hasProxy) {
			...
			config.keyCodes = new Proxy(config.keyCodes, {
				set: function set(target, key, value) {
					if (isBuiltInModifier(
						key)) { //匹配键盘上的快捷键 'stop,prevent,self,ctrl,shift,alt,meta,exact'
...

				{
					...
					if (
						...
						tip(
							...
							"templates. You should probably use \"" + altKey + "\" instead of \"" +
							key + "\"."
						);
					}
				}
	...
	function ensureCtor(comp, base) {
		...
		if (
			...
			(hasSymbol && comp[Symbol.toStringTag] ===
			'Module') //或者 支持hasSymbol 类型 并且判断 对象类的标签属性是Module "[object Module]"
		) {
			//将 comp 默认属性给 comp
			comp = comp.default;
		}
		...
		if (isDef(factory.contexts)) {
			...
			if (isObject(res)) { //如果是对象 表明支持promise
				...
				if (typeof res.then === 'function') {
					...
					'function') { //如果组件有定义并且有值，而且组件是异步的then是函数
	...
	function callUpdatedHooks(queue) {
		...
		while (i--) {
			...
			if (vm._watcher === watcher && vm
				._isMounted) { //判断watcher与vm._watcher 相等 _isMounted已经更新触发了 mounted 钩子函数
				//触发updated 更新数据钩子函数
				callHook(vm, 'updated');
			}
		}
	...
	var Watcher = function Watcher(
		...
		if (typeof expOrFn === 'function') {
			...
			//path 因该是路由地址
			if (bailRE.test(
				path)) { //  匹配上 返回 true     var bailRE = /[^\w.$]/;  //匹配不是 数字字母下划线 $符号   开头的为true
				return
			}
			...
			for (var i = 0; i < keys.length; i++) { //循环key
				...
				while (source) {
					if (source._provided && hasOwn(source._provided,
						provideKey)) { //判断_provided 存在么 并且是对象的时候，并且实例化属性provideKey 存在
	...
	function installRenderHelpers(target) {
		...
		target._b =
		bindObjectProps; //用于将v-bind="object"合并到VNode的数据中的运行时助手。  检查value 是否是对象，并且为value 添加update 事件
	...
	function initMixin(Vue) {
		Vue.prototype._init = function(options) { //初始化函数
			...
			initState(vm); //    //初始化状态
			initProvide(
			vm); // resolve provide after data/props  解决后提供数据/道具  provide 选项应该是一个对象或返回一个对象的函数。该对象包含可注入其子孙的属性，用于组件之间通信。
			...
			Vue[type] = function(
				...
				if (!definition) { //如果definition不存在
					...
					if (type === 'component' && isPlainObject(definition)) { //如果类型是组件
						...
						definition); // Class inheritance 类继承 用于vue多个组件中的合并拓展参数
		...
		render: function render() {
			...
			if (componentOptions) {
				...
				if (
					...
					//如果include存在,并且name不存在，或者name不存在include中则进if
					// excluded
					(exclude && name && matches(exclude,
					name)) //如果exclude存在 并且name存在 并且name存在exclude对象中
				) {
	...
	function createElement$1(tagName, vnode) {
		...
		if (vnode.data && vnode.data.attrs && vnode.data.attrs.multiple !==
			undefined) { //如果是select标签 判断是否设置了multiple属性。如果设置了则加上去
	...
	function createPatchFunction(backend) {
		...
		function invokeDestroyHook(vnode) {
			...
			if (isDef(data)) { //如果vonde有标签属性
				if (isDef(i = data.hook) && isDef(i = i
					.destroy)) { //如果有钩子函数，或者销毁的钩子函数destroy 就调用destroy或者钩子函数
					i(vnode);
				}
		...
		return function patch(
			...
			if (isUndef(vnode)) { //如果没有定义新的vonde
				if (isDef(oldVnode)) { //如果没有定义旧的vonde
					invokeDestroyHook(
					oldVnode); //如果vnode不存在但是oldVnode存在，说明意图是要销毁老节点，那么就调用invokeDestroyHook(oldVnode)来进行销毁
				}
				return
			}
			...
			var
		insertedVnodeQueue = []; //vonde队列 如果vnode上有insert钩子，那么就将这个vnode放入insertedVnodeQueue中作记录，到时再在全局批量调用insert钩子回调
			...
			if (isUndef(oldVnode)) { //如果没有定义旧的vonde
				...
				if (!isRealElement && //如果获取不到真实的dom 类型
					sameVnode(oldVnode,
					vnode) //sameVnode(oldVnode, vnode)2个节点的基本属性相同，那么就进入了2个节点的diff过程。
				) {
	...
	function updateAttrs(oldVnode, vnode) {
		...
		if (isDef(opts) && opts.Ctor.options.inheritAttrs ===
			false) { // 判断是否定义有拓展参数，并且需要Ctor.options.inheritAttrs 不等于 false的 时候才执行下面的代码
			return
		}
	...
	function addHandler(
		...
		var events;
		if (modifiers
			.native) { // 判断是有原生事件修饰符 通俗点讲：就是在父组件中给子组件绑定一个原生的事件，就将子组件变成了普通的HTML标签，不加'. native'事件是无法触  发的。
			/*
	...
	function genAssignmentCode(
		...
		if (res.key === null) {
			...
			return ("$set(" + (res.exp) + ", " + (res.key) + ", " + assignment +
				")") // 返回更新值 '$set(object[info],key,value)'
		}
	}
	...
	var autoCssTransition = cached(function(name) {
		return {
			...
			enterActiveClass: (name +
			"-enter-active"), //进入激活动画的css类   类似这样的 v-enter-active {transition: all .3s ease;}
	...
	function enter(
		...
		var enterActiveClass = data
		.enterActiveClass; //动画进入活跃的类  类似这样的    enter-active {transition: all .3s ease;}
	...
	var defaultTagRE = /\{\{((?:.|\n)+?)\}\}/g; //匹配viwe 视图中的{{指令}}
	var regexEscapeRE =
	/[-.*+?^${}()|[\]\/\\]/g; //匹配特殊符号  - 或者. 或者* 或者+ 或者? 或者^ 或者$ 或者{ 或者} 或者( 或者) 或者| 或者[ 或者] 或者/ 或者\
	...
	function parseText(text, //字符串
		...
		var tagRE = delimiters ? buildRegex(delimiters) :
		defaultTagRE; // 如果delimiters不存在则 用默认指令 {{}}，如果修改成其他指令则用其他指令
		...
		while ((match = tagRE.exec(
			text))) { //循环能匹配上的指令，全局匹配代码：的时候会有个lastIndex  执行exec方法后，lastIndex就会记录匹配的字符串在原始字符串中最后一位的索引加一，
			console.log('match=')
			console.log(match)
			...
			.length; // 上一次匹配出来的字符串的位置+上一次字符串的长度  比如字符串   我叫{{name}},今年{{age}},数据{{data.number}}个手机  这时候lastIndex 等于10
	...
	*/
	...
	")"; //  ((?:[a-zA-Z_][\\w\\-\\.]*\\:)?[a-zA-Z_][\\w\\-\\.]*)
	var startTagOpen = new RegExp(("^<" +
		qnameCapture)) // 匹配开头必需是< 后面可以忽略是任何字符串  ^<((?:[a-zA-Z_][\\w\\-\\.]*\\:)?[a-zA-Z_][\\w\\-\\.]*)
	...
	function parseHTML(
		...
		no; //函数匹配标签是否是 'area,base,br,col,embed,frame,hr,img,input,isindex,keygen, link,meta,param,source,track,wbr'
		var canBeLeftOpenTag$$1 = options.canBeLeftOpenTag ||
		no; //函数 //判断标签是否是 'colgroup,dd,dt,li,options,p,td,tfoot,th,thead,tr,source'
		var index = 0;
		var last, //
			lastTag; //
		console.log(html)
			...
			if (
				...
				if (textEnd === 0) { //标识是开始标签
					...
					if (conditionalComment.test(
						html)) { //匹配开始为 <![ 字符串  <![endif]-->   匹配这样动态加ie浏览器的 字符串  <!--[if IE 8]><link href="ie8only.css" rel="stylesheet"><![endif]-->
						//匹配ie浏览器动态加样式结束符号
		...
		function parseStartTag() {
			...
			startTagOpen); //匹配开始标签 匹配开头必需是< 后面可以忽略是任何字符串  ^<((?:[a-zA-Z_][\\w\\-\\.]*\\:)?[a-zA-Z_][\\w\\-\\.]*)
			console.log(start)
			...
			* */
			...
			if (expectHTML) { //true
				...
				if (
					...
					lastTag ===
					tagName //上一个标签和现在标签相同  <li><li> 编译成 <li></li>  但是这种情况是不会出现的 因为浏览器解析的时候会自动补全如果是<li>我是li标签<li> 浏览器自动解析成  <li>我是li标签</li><li> </li>
				) {
					//查找parseHTML的stack栈中与当前tagName标签名称相等的标签，
					//调用options.end函数，删除当前节点的子节点中的最后一个如果是空格或者空的文本节点则删除，
					//为stack出栈一个当前标签，为currentParent变量获取到当前节点的父节点
			...
			for (var i = 0; i < l; i++) {
				...
				attrs[i] = { //把数组对象属性值循环变成对象，这样可以过滤相同的属性
					...
					value: decodeAttr(value,
						shouldDecodeNewlines) //替换html 中的特殊符号，转义成js解析的字符串,替换 把   &lt;替换 <  ， &gt; 替换 > ， &quot;替换  "， &amp;替换 & ， &#10;替换\n  ，&#9;替换\t
	...
	function createASTElement(tag, //标签名称
		...
		return {
			...
			attrs), //对象属性 把数组对象转换成 对象  例如attrs = [{name:tag1,value:1},{ name:tag2,value:2},{name:tag3,value:3}]转换成map={tag1:1,tag2:2,tag3:3}
			parent: parent, //父层
			children: []
		}
	}
	...
	function parse(
		...
		* */
		...
		false; //模板编译器的选项。当使用默认的 vue-template-compiler 的时候，你可以使用这个选项来添加自定义编译器指令、模块或通过 { preserveWhitespace: false } 放弃模板标签之间的空格。
		var root;
		...
		parseHTML(
			template, //字符串模板
			{
				...
				isUnaryTag: options
				.isUnaryTag, //匹配标签是否是 'area,base,br,col,embed,frame,hr,img,input,isindex,keygen, link,meta,param,source,track,wbr'
				canBeLeftOpenTag: options
				.canBeLeftOpenTag, //判断标签是否是 'colgroup,dd,dt,li,options,p,td,tfoot,th,thead,tr,source'
				shouldDecodeNewlines: options.shouldDecodeNewlines, //flase //IE在属性值中编码换行，而其他浏览器则不会
				shouldDecodeNewlinesForHref: options
				...
				start: function start(
					...
					if (!root) {
						...
						if (root.if && (element.elseif || element.else)) {
							...
							element); //检查根约束 根节点不能是slot或者template标签，并且不能含有v-for 属性
					...
					if (
						currentParent &&
						!element
						.forbidden //如果是style或者是是script 标签并且type属性不存在 或者存在并且是javascript 属性 的时候返回真
					) {
				...
				chars: function chars(text) {
					...
					if (text) {
						...
						if (
							...
							text !== ' ' && //你是空节点
							(res = parseText(text,
							delimiters)) //匹配view 指令，并且把他转换成 虚拟dom vonde 需要渲染的函数,比如指令{{name}}转换成 _s(name)
							//比如字符串  我叫{{name}},今年{{age}},数据{{data.number}}个手机  转换成 我叫+_s(name)+,今年+_s(age)+,数据+_s(data.number)+个手机
						) {
							console.log(res)
							...
							children.push({ //添加为属性节点
								type: 2, //Attr	代表属性
								expression: res
								.expression, //比如字符串  我叫{{name}},今年{{age}},数据{{data.number}}个手机  转换成 我叫+_s(name)+,今年+_s(age)+,数据+_s(data.number)+个手机
								tokens: res.tokens,
								text: text //html文本
							});
						} else if (
	...
	function parseFor(
		...
		forAliasRE); //匹配 含有   字符串 in  字符串   或者  字符串 of  字符串 比如(value, key, index) in data
		...
		''); //去除括号 比如(value, key, index) in data 变成 value, key, index


		var iteratorMatch = alias.match(
		forIteratorRE); // 匹配出分组 [0: ", key, index",  1: " key" , 2: "index"]
	...
	function processComponent(el) {
		...
		if (getAndRemoveAttr(el, 'inline-template') !=
			null) { //当 inline-template 这个特殊的特性出现在一个子组件上时，这个组件将会使用其里面的内容作为模板，而不是将其作为被分发的内容。这使得模板的撰写工作更加灵活。
		...
		for (i = 0, l = list.length; i < l; i++) { //循环属性列表
			...
			if (dirRE.test(name)) { // 判断是否是 v-或者@或者:  属性开头的
...

				{
					...
					delimiters); //校验是否含有{{}} 括号的属性 比如 <div  style='style'>  </div> 如果写成 <div  style='{{style}}'>  </div>  则报错警告
					if (res) {
						warn$2(
							name + "=\"" + value + "\": " +
							'Interpolation inside attributes has been removed. ' +
	...
	function preTransformNode(
		...
		if (el.tag === 'input') { //如果是input标签
			...
			if (typeBinding) { //判断 typeBinding 是否存在
				...
				""; //判断if是否有值比如v-if="flag" 如果有 变成   &&(flag)
				var hasElse = getAndRemoveAttr(el, 'v-else', true) !=
				null; //获取 v-else 属性值 标志 如果有有 可能是 ''   ，  ''!= null 为真
				...
				ifConditionExtra; //     ifConditionExtra 是 判断if是否有值比如v-if="flag" 如果有 变成   &&(flag)  最终合并成  ((abc).type)===checkbox&&(flag)
				//为if指令添加标记
				addIfCondition(
					branch0, //虚拟dom
					{
	...
	var baseOptions = {
		...
		staticKeys: genStaticKeys(
			modules$1) // * 把数组对象 [{ staticKeys:1},{staticKeys:2},{staticKeys:3}]连接数组对象中的 staticKeys key值，连接成一个字符串 str=‘1,2,3’
	};
	...
	function isStatic(node) {
		...
		return !!(
			...
			(
				...
				// not a component 不是一个组件   保留标签 判断是不是真的是 html 原有的标签 或者svg标签
				!isDirectChildOfTemplateFor(node) &&
				//    判断当前ast 虚拟dom 的父标签 如果不是template则返回false，如果含有v-for则返回true
				Object.keys(node).every(
				isStaticKey) //node的key必须每一项都符合   匹配type,tag,attrsList,attrsMap,plain,parent,children,attrs + staticKeys 的字符串
			)
		)
	}
	...
	function checkEvent(exp, text, errors) {
		...
		var keywordMatch = stipped.match(
		unaryOperatorsRE); //匹配是否含有delete (任何字符) 或  typeof (任何字符) 或  void (任何字符) 关键词
	...
	function checkIdentifier(ident, //识别
		...
		if (typeof ident === 'string') {
			try {
				...
				"=_")); //检查var a ='_' 或者 检查var a =_  是否会报错  new function  用来检测js错误 与eval差不多
			} catch (e) {
